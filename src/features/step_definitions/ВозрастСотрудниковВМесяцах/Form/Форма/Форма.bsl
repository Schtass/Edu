...
Процедура ВыдатьОшибкуОНеправильномВозрасте() Экспорт
	...
	Попытка
		...
		ПолучитьВозратСотрудникаВМесяцахНаДату(Контекст.Сотрудник.Ссылка, Контекст.ДатаПроверки);
		
		ВызватьИсключение "Функция ПолучитьВозратСотрудникаВМесяцахНаДату() должна вызвать исключение, но его нет!";
		
	Исключение
		
		ОписаниеОшибки = ОписаниеОшибки();
		
		Если СтрНайти(ОписаниеОшибки, "неправильно указана дата рождения!") = 0 Тогда 
...
Процедура ВозратСотрудникаВМесяцахОтрицательный() Экспорт
	...
	Если Контекст.Сотрудник.ДатаРождения < Контекст.ДатаПроверки Тогда 
		
		ВызватьИсключение "Тестовый сотрудник " + Контекст.Сотрудник.Ссылка
							+ " на дату " + Контекст.ДатаПроверки + " уже родился!";
...
Функция ПолучитьВозратСотрудникаВМесяцахНаДату(Сотрудник, Дата) 
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ФизическиеЛица.Наименование КАК Сотрудник,
		|	0 КАК Возраст
		|ИЗ
		...
		|			ИЛИ РАЗНОСТЬДАТ(ФизическиеЛица.ДатаРождения, &Дата, ГОД) > 100
		|			ИЛИ ФизическиеЛица.ДатаРождения > &Дата)";
	...
	Если Выборка.Следующий() Тогда 
		...
		Если Выборка.Возраст = 0 Тогда 
			
			ВызватьИсключение "У сотрудника " + Выборка.Сотрудник + "неправильно указана дата рождения!";
